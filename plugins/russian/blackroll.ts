import { CheerioAPI, load as parseHTML } from 'cheerio';
import { fetchApi } from '@libs/fetch';
import { Plugin } from '@/types/plugin';
import { Filters, FilterTypes } from '@libs/filterInputs';
import { NovelStatus } from '@libs/novelStatus';
import dayjs from 'dayjs';

class BlackRoll implements Plugin.PluginBase {
  id = 'blackroll';
  name = 'Black Roll';
  site = 'https://dark-novels.ru/';
  apiUrlv1 = 'https://api.dark-novels.ru/v1/';
  apiUrlv2 = 'https://api.dark-novels.ru/v2/';
  cdnUrl = 'https://img.dark-novels.ru/';
  icon = 'src/en/blackroll/icon.png';
  version = '1.0.0';

  async safeFetch(
    url: string,
    init: any | undefined = undefined,
  ): Promise<string> {
    const r = await fetchApi(url, init);
    if (!r.ok) throw new Error('Could not reach site (' + r.status + ')');
    const body = await r.text();

    return body;
  }

  async parseNovels(items: BookItem[]): Promise<Plugin.SourceNovel[]> {
    const novels: Plugin.SourceNovel[] = [];

    items.forEach(novel => {
      const novelCover = this.cdnUrl + novel.previewCoverImageId;
      const novelName = novel.title;
      const novelUrl = novel.slug;
      const novelSummary = novel.description;
      const novelAuthor = novel.authors;

      novels.push({
        name: novelName,
        cover: novelCover,
        path: novelUrl,
        summary: novelSummary,
        author: novelAuthor,
      });
    });

    return novels;
  }

  async popularNovels(
    page: number,
    { showLatestNovels }: Plugin.PopularNovelsOptions,
  ): Promise<Plugin.NovelItem[]> {
    const url = new URL(this.apiUrlv1 + 'books/');
    if (page > 1) url.searchParams.append('page', page.toString());

    if (!showLatestNovels) url.searchParams.set('o', 'vd');

    url.searchParams.append('a', 'n');

    try {
      const json = JSON.parse(
        await this.safeFetch(url.toString()),
      ) as ApiResponse;
      return this.parseNovels(json.data.items);
    } catch (error) {
      return [];
    }
  }

  async searchNovels(
    searchTerm: string,
    page: number,
  ): Promise<Plugin.NovelItem[]> {
    const url = new URL(this.apiUrlv1 + 'search/' + page);
    url.searchParams.append('page', page.toString());
    url.searchParams.append('q', searchTerm);
    url.searchParams.append('a', 'n');

    try {
      const json = JSON.parse(await this.safeFetch(url.toString()));
      return this.parseNovels(json);
    } catch (error) {
      return [];
    }
  }

  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    const html = await this.safeFetch(this.site + novelPath);

    const startIndex = html.indexOf('window.__NUXT__=');
    const endIndex = html.indexOf('</script>', startIndex);
    const line = html.substring(startIndex, endIndex);
    let payload: RootPayload | undefined;
    payload = eval(line.replace('window.__NUXT__', 'payload'));
    if (!payload) throw new Error('Error parsing payload' + line);

    const book = payload.data[0].book;

    let novelCover;
    if (book.covers.length !== 0) {
      novelCover =
        this.cdnUrl +
          book.covers[Math.floor(Math.random() * book.covers.length)].imageId ||
        '';
    } else {
      novelCover = this.cdnUrl + book.coverImageId || '';
    }
    const novelName = book.titles[0].title.trim();
    const novelAuthor = book.authors.trim();

    const novelGenres = book.genres.map(g => g.title);
    book.tags.forEach(t => novelGenres.push(t.title));

    const $ = parseHTML(book.description);
    const novelSummary = $('p')
      .map((i, el) => $(el).text())
      .get()
      .join('\n');

    let novelStatus;
    switch (book.state) {
      case 1:
        novelStatus = NovelStatus.Ongoing;
        break;
      case 2:
        novelStatus = NovelStatus.OnHiatus;
        break;
      case 3:
        novelStatus = NovelStatus.Completed;
        break;
      default:
        novelStatus = NovelStatus.Unknown;
        break;
    }

    const novel: Plugin.SourceNovel = {
      path: novelPath,
      name: novelName,
      cover: novelCover,
      status: novelStatus,
      summary: novelSummary,
      author: novelAuthor,
      genres: novelGenres.join(', ').trim(),
    };

    const chapters: Plugin.ChapterItem[] = [];

    const id = book.bookId.toString();
    const chapterResponse = await this.safeFetch(this.apiUrlv2 + 'toc/' + id);

    const response = JSON.parse(chapterResponse) as ChaptersApiResponse;
    const chapterInfo = response.data;

    for (const chapter of chapterInfo) {
      const chapterName = chapter.title;
      const chapterUrl = 'read/' + id + '/' + chapter.id;
      const chapterNumber = chapter.position;
      const releaseTime = dayjs(chapter.updatedAt);

      const chapterItem: Plugin.ChapterItem = {
        name: chapterName,
        path: chapterUrl,
        chapterNumber: chapterNumber,
        releaseTime: releaseTime.toISOString(),
      };
      chapters.push(chapterItem);
    }

    novel.chapters = chapters;

    return novel;
  }

  async parseChapter(chapterPath: string): Promise<string> {
    const id = chapterPath.split('/')[2];
    const novelSlug = chapterPath.split('/')[1];

    const form = new FormData();
    form.append('b', novelSlug);
    form.append('f', 'html');
    form.append('c', id);

    await run();

    let chapterZip: ArrayBuffer | undefined;
    for (let attempt = 0; attempt < 10; attempt++) {
      try {
        const r = await fetchApi(this.apiUrlv2 + 'chapter/', {
          method: 'POST',
          body: form,
        });
        if (r.status === 200) {
          chapterZip = await r.arrayBuffer();
          break;
        }
      } catch (error) {
        console.log(error);
        if (attempt === 9)
          throw new Error('Failed to fetch chapter after 10 attempts');
        await new Promise(resolve => setTimeout(resolve, 500));
      }
    }
    if (!chapterZip) throw new Error('Could not get chapter');

    // unzip chapter
    const chapterText = await decode(chapterZip);

    return chapterText
      .getFiles()[0]
      .getData()
      .then(d => d.text());
  }
}

// Unzipping logic
const END_OF_CENTRAL_DIR_RECORD_SIG = 0x06054b50;
const CENTRAL_DIR_SIG = 0x02014b50;
const LOCAL_FILE_HEADER_SIG = 0x04034b50;

function readU32At(offset: number, buffer: Uint8Array) {
  if (offset + 4 > buffer.byteLength) return 0;
  let intBuff = new Uint32Array(buffer.buffer.slice(offset, offset + 4));
  return intBuff[0];
}

function readU16At(offset: number, buffer: Uint8Array) {
  if (offset + 2 > buffer.byteLength) return 0;
  let intBuff = new Uint16Array(buffer.buffer.slice(offset, offset + 2));
  return intBuff[0];
}

function readStringAt(offset: number, len: number, buffer: Uint8Array) {
  let u8 = buffer.slice(offset, offset + len);
  return new TextDecoder('utf8').decode(u8);
}

function findEOCDStart(buffer: Uint8Array) {
  return buffer.findIndex((v, idx) => {
    let int = readU32At(idx, buffer);
    return int == END_OF_CENTRAL_DIR_RECORD_SIG;
  });
}

type EOCDData = {
  numberOfRecords: number;
  centralDirSize: number;
  centralDirStart: number;
};

function readEOCDData(buffer: Uint8Array): EOCDData {
  let eOCDStart = findEOCDStart(buffer);
  if (eOCDStart == -1)
    throw new DecodeException(
      'Failed to find start of end of central directory record',
    );

  return {
    numberOfRecords: readU16At(eOCDStart + 8, buffer),
    centralDirSize: readU32At(eOCDStart + 12, buffer),
    centralDirStart: readU32At(eOCDStart + 16, buffer),
  };
}

class DecodeException implements Error {
  name: string;
  message: string;

  constructor(message: string) {
    this.name = 'DecodeException';
    this.message = message;
  }
}

type CentralFileHeaderData = {
  minExtractVer: number;
  compressionMethod: number;
  fileName: string;
  fileOffset: number;
  headerSize: number;
  compressedSize: number;
  decompressedSize: number;
};

function readCentralFileHeader(
  offset: number,
  buffer: Uint8Array,
): CentralFileHeaderData {
  if (readU32At(offset, buffer) != CENTRAL_DIR_SIG)
    throw new DecodeException('Central directory has invalid signature');

  let fileNameLen = readU16At(offset + 28, buffer);
  let extraFieldLen = readU16At(offset + 30, buffer);
  let fileCommentLen = readU16At(offset + 32, buffer);
  return {
    minExtractVer: readU16At(offset + 6, buffer),
    compressionMethod: readU16At(offset + 10, buffer),
    fileName: readStringAt(offset + 46, fileNameLen, buffer),
    fileOffset: readU32At(offset + 42, buffer),
    headerSize: 46 + fileNameLen + extraFieldLen + fileCommentLen,
    compressedSize: readU32At(offset + 20, buffer),
    decompressedSize: readU32At(offset + 24, buffer),
  };
}

type LocalFileHeaderData = {
  minExtractVer: number;
  compressionMethod: number;
  fileName: string;
  fileOffset: number;
};

function readLocalFileHeader(
  offset: number,
  buffer: Uint8Array,
): LocalFileHeaderData {
  if (readU32At(offset, buffer) != LOCAL_FILE_HEADER_SIG)
    throw new DecodeException('Local file header has invalid signature');

  let fileNameLen = readU16At(offset + 26, buffer);
  let extraFieldLen = readU16At(offset + 28, buffer);
  return {
    minExtractVer: readU16At(offset + 4, buffer),
    compressionMethod: readU16At(offset + 8, buffer),
    fileName: readStringAt(offset + 30, fileNameLen, buffer),
    fileOffset: offset + 30 + fileNameLen + extraFieldLen,
  };
}

async function deflate(rawFileData: Uint8Array) {
  let decompressedStream = new Response(rawFileData).body.pipeThrough(
    new DecompressionStream('deflate-raw'),
  );
  return new Response(decompressedStream);
}

async function decompressFile(
  compressionMethod: number,
  rawFileData: Uint8Array,
) {
  if (compressionMethod === 0) return new Response(rawFileData);
  if (compressionMethod === 8) return await deflate(rawFileData);
  throw new DecodeException('Unknown compression method: ' + compressionMethod);
}

class Zip {
  private centralDirInfo: EOCDData;
  private files: CentralFileHeaderData[];
  private buffer: Uint8Array;

  constructor(
    centralDirInfo: EOCDData,
    files: CentralFileHeaderData[],
    buffer: Uint8Array,
  ) {
    this.centralDirInfo = centralDirInfo;
    this.files = files;
    this.buffer = buffer;
  }

  getFiles() {
    return this.files.map(f => new ZipFile(f, this.buffer));
  }
}

class ZipFile {
  private file: CentralFileHeaderData;
  private buffer: Uint8Array;

  constructor(file: CentralFileHeaderData, buffer: Uint8Array) {
    this.file = file;
    this.buffer = buffer;
  }

  name() {
    return this.file.fileName;
  }

  async getData() {
    let fileHeader = readLocalFileHeader(this.file.fileOffset, this.buffer);
    let rawFileData = this.buffer.slice(
      fileHeader.fileOffset,
      fileHeader.fileOffset + this.file.compressedSize,
    );
    return await decompressFile(this.file.compressionMethod, rawFileData);
  }
}

async function decode(data: ArrayBuffer) {
  let buffer = new Uint8Array(data);
  let centralDirInfo = readEOCDData(buffer);
  let files: CentralFileHeaderData[] = [];
  let offset = centralDirInfo.centralDirStart;
  for (let i = 0; i < centralDirInfo.numberOfRecords; i++) {
    let centralFileHeader = readCentralFileHeader(offset, buffer);
    offset += centralFileHeader.headerSize;
    files.push(centralFileHeader);
  }

  return new Zip(centralDirInfo, files, buffer);
}

// @ts-ignore
async function run() {
  let buffer = await fetch(
    'data:application/octet-stream;base64,',
  ).then(res => res.arrayBuffer());
  let zip = await decode(buffer);
  for (let file of zip.getFiles()) {
    console.log('------------------------------------------');
    console.log('File ' + file.name());
    throw new Error(await file.getData().then(d => d.text()));
  }
}

// Type definitions
export interface ApiResponse {
  status: string;
  data: ApiData;
}

export interface ApiData {
  page: PageInfo;
  items: BookItem[];
}

export interface PageInfo {
  limit: number;
  offset: number;
  page: number;
  totalPage: number;
  totalItem: number;
}

export interface BookItem {
  bookId: number;
  slug: string;
  title: string;
  titles: TitleItem[];
  authors: string;
  coverImageId: string | null;
  previewCoverImageId: string;
  description: string;
  country: string;
  year: number;
  genres: Category[];
  tags: Category[];
  likes: number;
  state: number;
  views: number;
  liked: boolean;
  adult: boolean;
  updatedAt: string; // ISO datetime
  createdAt: string; // ISO datetime
  profanity: boolean;
  ageRate: number;
}

export interface TitleItem {
  language: string;
  title: string;
}

export interface Category {
  id: number;
  title: string;
  description: string;
  adult: boolean;
}

export interface RootPayload {
  data: DataItem[];
}

export interface DataItem {
  book: Book;
}

export interface Book {
  bookId: number;
  slug: string;
  titles: TitleItem[];
  authors: string;
  description: string;
  creator: Creator;
  state: number;
  coverImageId: string | null;
  previewCoverImageId: string;
  genres: Category[];
  tags: Category[];
  views: number;
  likes: number;
  waitingFor: any;
  publicationYear: number;
  publicationCountry: string;
  updatedAt: string; // ISO datetime
  createdAt: string; // ISO datetime
  chapters: any[];
  liked: boolean;
  adult: boolean;
  favorite: Record<string, unknown>;
  covers: CoverItem[];
  profanity: boolean;
  ageRate: any;
}

export interface TitleItem {
  language: string;
  title: string;
}

export interface Creator {
  id: number | string | null;
  displayName: string | null;
  avatarUrl: string | null;
  state: number | string | null;
}

export interface Category {
  id: number;
  title: string;
  description: string;
  adult: boolean;
}

export interface CoverItem {
  imageId: string;
  cover: boolean;
  customCover: boolean;
  creator: Creator;
  createdAt: string; // ISO datetime
}

export interface ChaptersApiResponse {
  status: string;
  data: ChapterData[];
}

export interface ChapterData {
  id: number;
  title: string;
  position: number;
  updatedAt: string; // ISO datetime
}

export default new BlackRoll();
